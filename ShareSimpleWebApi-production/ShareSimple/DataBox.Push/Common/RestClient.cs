    ...
    public class RestClient
    {
        ...
        private static HttpWebRequest CreateWebRequest(string endpoint, string method = "GET", Dictionary<string, string> headers = null)
        {
            ...
            if (headers != null && headers.Count > 0)
            {
                ...
                {
                    if (headerName.ToLower().Equals("content-type"))
                        webRequest.ContentType = headers[headerName];
                    else if (headerName.ToLower().Equals("accept"))
                        webRequest.Accept = headers[headerName];
                    else if (headerName.ToLower().Equals("user-agent"))
                        webRequest.UserAgent = headers[headerName];
                    else
