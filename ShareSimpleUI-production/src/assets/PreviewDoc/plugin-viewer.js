...
function ODFViewerPlugin() {
    ...
    this.initialize = function (q, a) {
        b(function () {
            ...
            f.addListener("statereadychange", function () {
                var textDOMList = document.getElementsByTagName("text:span");
                if (!!textDOMList) {
                    for (var t = 0; t < textDOMList.length; t++) {
                        if (!!textDOMList[t]
                            && textDOMList[t].textContent
                            && textDOMList[t].textContent.indexOf('Evaluation Warning')===0) {
                            textDOMList[t].textContent = '';
                            break;
                        }
                    }
                }
                var excelDOMList = document.getElementsByTagName("table:table");
                if (!!excelDOMList) {
                    for (var ex = 0; ex < excelDOMList.length; ex++) {
                        if (!!excelDOMList[ex]
                            && !!excelDOMList[ex].getAttribute("table:name")
                            && excelDOMList[ex].getAttribute("table:name") === 'Evaluation Warning') {
                            document.getElementsByTagName("table:table")[ex].innerHTML = '';
                            break;
                        }
                    }
                }
    ...
    this.getPages = function () {
        var b = Array.prototype.slice.call(s.getElementsByTagNameNS("urn:oasis:names:tc:opendocument:xmlns:drawing:1.0", "page")), a = [], e, c;
        for (e = 0; e < b.length; e += 1)
            c = [b[e].getAttribute("draw:name"), b[e]],
            a.push(c);
        return a
    }
    ;
...
function PDFViewerPlugin() {
    ...
    function s(d) {
        var a, b;
        t[d.pageIndex] === l.BLANK && (t[d.pageIndex] = l.RUNNING,
        a = c[d.pageIndex],
        b = B[d.pageIndex],
        ...
        d.render({
            ...
            t[d.pageIndex] === l.RUNNINGOUTDATED ? (t[d.pageIndex] = l.BLANK,
            s(d)) : t[d.pageIndex] = l.FINISHED
        }))
    }
    function k() {
        var d = !a.isSlideshow();
    ...
    this.initialize = function (a, b) {
        ...
        r(function () {
            ...
            PDFJS.getDocument(b).then(function (b) {
                v = b;
                u = a;
                for (c = 0; c < v.numPages; c += 1)
                    v.getPage(c + 1).then(q)
            })
        })
    }
    ;
    ...
    ;
    this.showPage = function (b) {
        a.isSlideshow() ? (c[A - 1].style.display = "none",
        A = b,
        s(e[b - 1]),
        c[b - 1].style.display = "block") : (b = c[b - 1],
...
function Viewer(b, r) {
    function f() {
        var a, c, d, e, g, f;
        f = "undefined" !== String(typeof PreviewDoc_version) ? PreviewDoc_version : "From Source";
        b && (d = b.getPluginName(),
        e = b.getPluginVersion(),
        g = b.getPluginURL());
        ...
        I.id = "aboutDialog";
        I.innerHTML = '<h1>PreviewDoc</h1><p>Open Source document viewer for webpages, built with HTML and JavaScript.</p><p>Learn more and get your own copy on the <a href="http://PreviewDoc.org/" target="_blank">PreviewDoc website</a>.</p>' + (b ? '<p>Using the <a href = "' + g + '" target="_blank">' + d + '</a> (<span id = "pluginVersion">' + e + "</span>) plugin to show you this document.</p>" : "") + "<p>Version " + f + '</p><p>Supported by <a href="https://nlnet.nl" target="_blank"><br><img src="images/nlnet.png" width="160" height="60" alt="NLnet Foundation"></a></p><p>Made by <a href="http://kogmbh.com" target="_blank"><br><img src="images/kogmbh.png" width="172" height="40" alt="KO GmbH"></a></p><button id = "aboutDialogCloseButton" class = "toolbarButton textButton">Close</button>';
        K.appendChild(a);
        a.appendChild(c);
        c.appendChild(I);
    ...
    function x(a) {
        ...
        for (e = 0; e < b.length; e += 1)
            c = b[e],
            c.value !== a ? c.selected = !1 : d = c.selected = !0;
        return d
    }
    function s(a, b) {
        if (a !== g.getZoomLevel()) {
    ...
    function a(a, c) {
        var d, e;
        if (d = "custom" === a ? parseFloat(document.getElementById("customScaleOption").textContent) / 100 : parseFloat(a))
            s(d, !0);
        else {
            d = h.clientWidth - A;
            e = h.clientHeight - A;
    ...
    this.initialize = function () {
        ...
        b.onLoad = function () {
            ...
            b.isSlideshow() ? (h.classList.add("slideshow"),
            P.style.visibility = "visible") : (S.style.visibility = "visible",
            b.getPageInView && (P.style.visibility = "visible"));
            L = !0;
            H = b.getPages();
    ...
    this.download = function () {
        var a = J.split("#")[0];
        var targetedFile = DownloadActualTargetFile(a);
        if (!!targetedFile) {
            window.open(targetedFile + "#viewer.action=download", "_blank");
        }
        else {
            window.open(a + "#viewer.action=download", "_blank");
        }
        
    };

    function DownloadActualTargetFile(targetFileName) {
        var i, frames, actualExtension, opensourceExtension;
        frames = parent.document.getElementsByTagName("iframe");
        for (i = 0; i < frames.length; ++i) {
            if (!!frames[i].name && getOnlyFileNameWithoutExtension(frames[i].name) === getOnlyFileNameWithoutExtension(targetFileName.replace(/^.*[\\\/]/, ''))) {
                actualExtension = frames[i].name.split('.').pop();
                opensourceExtension = targetFileName.split('.').pop();
                return targetFileName.replace(opensourceExtension, actualExtension);
            }
        }
    }
    function getOnlyFileNameWithoutExtension(fileName) {
        ...
        return fileName.substr(fileNameIndex, dotIndex < fileNameIndex ? fileName.length : dotIndex);
    }
    

    this.toggleFullScreen = function () {
        D ? document.exitFullscreen ? document.exitFullscreen() : document.cancelFullScreen ? document.cancelFullScreen() : document.mozCancelFullScreen ? document.mozCancelFullScreen() : document.webkitExitFullscreen ? document.webkitExitFullscreen() : document.webkitCancelFullScreen ? document.webkitCancelFullScreen() : document.msExitFullscreen && document.msExitFullscreen() : m.requestFullscreen ? m.requestFullscreen() : m.mozRequestFullScreen ? m.mozRequestFullScreen() : m.webkitRequestFullscreen ? m.webkitRequestFullscreen() : m.webkitRequestFullScreen ? m.webkitRequestFullScreen(Element.ALLOW_KEYBOARD_INPUT) : m.msRequestFullscreen && m.msRequestFullscreen()
    }
    ;
    this.togglePresentationMode = function () {
        var b = document.getElementById("overlayCloseButton");
        d ? ("block" === blanked.style.display && (blanked.style.display = "none",
        u()),
        ...
        ,
        a("auto")) : (C.style.display = G.style.display = "none",
    ...
    (function () {
        f();
        b && (g.initialize(),
        document.exitFullscreen || document.cancelFullScreen || document.mozCancelFullScreen || document.webkitExitFullscreen || document.webkitCancelFullScreen || document.msExitFullscreen || (document.getElementById("fullscreen").style.visibility = "hidden",
        ...
        window.addEventListener("scalechange", function (a) {
            var b = document.getElementById("customScaleOption")
              , c = x(String(a.scale));
            b.selected = !1;
            c || (b.textContent = Math.round(1E4 * a.scale) / 100 + "%",
            b.selected = !0)
        }, !0),
...
; (function () {
    function b(a, b) {
        ...
        c.onreadystatechange = function () {
            ...
            4 === c.readyState && ((200 <= c.status && 300 > c.status || 0 === c.status) && (a = c.getResponseHeader("content-type")) && q.some(function (b) {
                return b.supportsMimetype(a) ? (f = b,
                console.log("Found plugin by mimetype and xhr head: " + a),
                !0) : !1
            }),
            b(f))
        }
    ...
    function r(a) {
        var b;
        q.some(function (c) {
            return c.supportsFileExtension(a) ? (b = c,
            !0) : !1
        });
        return b
    }
    ...
    function isImage(fileUrl) {
        ...
        var fileType = fileUrl.substring(fileUrl.lastIndexOf('.') + 1, fileUrl.length) || fileUrl;
        if (allowedExtension.indexOf(fileType.toLowerCase()) > -1) {
            return true;
        }
        return false;
    }

    function isTextFile(fileUrl) {
    ...
    function showImage(imageUrl) {
        var fullscreenIcon = document.getElementById('titlebarRight');
        fullscreenIcon.style.display="none";
        var canvasContainer = document.getElementById('canvasContainer');
        var imageContainer = document.createElement("div");
        imageContainer.className = "image-container-custom-preview-doc";
        var img = document.createElement("img");
        img.className = "image-custom-preview-doc";
        img.src = imageUrl;
        img.style.width = "100%";
        imageContainer.appendChild(img);
        canvasContainer.appendChild(imageContainer);
    }
    function showText(fileUrl) {
    ...
    function showFileContent(text) {
        ...
        div_h.id = "pageContainer" + a;
        div_h.className = "page";
        div_h.style.width = "612px";
        div_h.style.minHeight = "792px";
        div_h.style.display = "inline-block";
        var text_layer_b = document.createElement("div");
        text_layer_b.className = "textLayer";
        text_layer_b.id = "textLayer" + a;

        var textChild = document.createElement("div");
        textChild.className = "text-custom-preview-doc";
        textChild.style.minWidth = "612px";
        textChild.style.minHeight = "792px";
        textChild.style.backgroundColor = "white";
        var textContent = document.createTextNode(text);
        textChild.appendChild(textContent);

        div_h.appendChild(textChild);
        canvasContainer.appendChild(div_h);
    }
    ...
    window.onload = function () {
        var a = document.location.hash.substring(1), e = s(document.location), c;
        if (isImage(a)) {
            showImage(a);
            return;
        }
